...
<div class="card">
  <div class="card-body">
    <h5 class="card-title">Dashboard/ajout commission</h5>
    <a href="{{route('commissions')}}"><button class="btn btn-dark"><i class="fa fa-arrow-left" aria-hidden="true"></i> Retour</button></a><br><br>
            @if(session()->has('status'))
                ...
                @endif
                    <form method="POST" enctype="multipart/form-data" action="{{route('save_commission')}}">
                    @csrf
                    <div class="row">
                        <div class="col">
                            <label for="exampleInputEmail1" class="form-label">Montant </label>
                            <input  required  type="text" name="montant" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                        </div>
                        <div class="col">
                            <label for="exampleInputEmail1" class="form-label">Pourcentage</label>
                            <input required  type="text" name="pourcentage" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                        </div>
                        <div class="col">
                        <label for="exampleInputEmail1" class="form-label">Agence partenaire</label>
                        <select name="agence" class="form-select form-select-sm" aria-label=".form-select-sm example" required>
