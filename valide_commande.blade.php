...
<div class="card">
  <div class="card-body">
    <h5 class="card-title">Dashboard/validation commande</h5>
    <a href="{{route('commande')}}"><button class="btn btn-dark"><i class="fa fa-arrow-left" aria-hidden="true"></i> Retour</button></a><br><br>
            @if(session()->has('status'))
                ...
                @endif
                    <form method="POST" enctype="multipart/form-data" action="{{route('save_valide_commande',['id'=>$id])}}">
                    @csrf
                    <div class="row">
                        <div class="col">
                            <label for="exampleInputEmail1" class="form-label">Agence partenaire </label>
                            <select required name="agence" class="form-select" aria-label="Default select example">
                        ...
                        <div class="col">
                            <label for="exampleInputEmail1" class="form-label">Montant commission</label>
                            <input required  type="text" name="prix" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                            </select>                        
                    </div>
                    </div>
